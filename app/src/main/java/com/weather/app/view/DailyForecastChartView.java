 ...
 */
public class DailyForecastChartView {
...
//            "fonts/NotoSansCJKsc-Thin.otf");
    ...
    public GraphicalView getChartGraphicalView(Context context,
        ...
        return ChartFactory.getCubeLineChartView(context,getDataSet(titles, xValues, yValues),
                getRenderer(titles, mStrDates,yValues,tf),0.4f);
    }
    ...
    public XYMultipleSeriesDataset getDataSet(String[] titles,
        ...
        for (int i = 0; i < titles.length; i++){
            ...
            timeSeries.add(xV[0],yV[0]-2);
            for (int k = 0; k < xV.length-1; k++) {
...
//        mRenderer.setYAxisMin(getMin(yValues.get(1)));//设置y轴最小值是0
//        mRenderer.setYAxisMax(getMax(yValues.get(0)) + 2);
        mRenderer.setYAxisMin(getMin(yValues.get(1)) - Math.abs(getMin(yValues.get(1))*0.05));//设置y轴最小值是0
        mRenderer.setYAxisMax(getMax(yValues.get(0)) + 1.5);
...
//        mRenderer.setShowGrid(true);//显示网格
        ...
        LogUtil.i("TAG","temp is " + yValues.get(1).toString());

        //将x标签栏目显示如：1,2,3,4替换为显示1月，2月，3月，4月
        mRenderer.addXTextLabel(1, "今天");
        mRenderer.addXTextLabel(2, "明天");
...
//        mRenderer.setMargins(new int[]{5, 5, 5, 5});//设置视图位置
        mRenderer.setMargins(new int[]{0,0,0,0});//设置视图位置
...
//        mRenderer.setLabelFormat();
        ...
        for (i = 0; i < titles.length; i++) {
            ...
            xySeriesRenderer.setLineWidth(1.0f);//设置线宽
...
//                case 1:
//                    xySeriesRenderer.setFillBelowLineColor(MyApplication.getContext().getResources()
//                            .getColor(R.color.low_temp_area_color));
//
//            }
